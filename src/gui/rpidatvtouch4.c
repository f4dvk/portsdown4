...
char TabFreq[9][255]={"71", "145.9", "437", "1249", "1255", "436", "436.5", "437.5", "438"};
...
void RTLstart()
{
  ...
  if(RTLdetected == 1)
  {
    ...
    {
      strcat(rtlcall, " | sox --buffer 32 -t raw -r 16k -b 16 -c 1 -e signed-integer - -r 48k -t raw - 2>/dev/null");
      ...
      if (strcmp(RTLmode[0], "dstar") == 0)
      {
		...
		if ((strcmp(RTLmode[0], "am") == 0) || (strcmp(RTLmode[0], "fm") == 0))
    {
      strcat(rtlcall, " | sox --buffer 32 -t raw -r 12k -e s -b 16 -c 1 - -t wav - 2>/dev/null");
    }
    if (strcmp(RTLmode[0], "wbfm") == 0)
    {
      strcat(rtlcall, " | sox --buffer 32 -t raw -r 32k -e s -b 16 -c 1 - -t wav - 2>/dev/null");
    }
    if ((strcmp(RTLmode[0], "usb") == 0) || (strcmp(RTLmode[0], "lsb") == 0))
    {
      strcat(rtlcall, " | sox --buffer 32 -t raw -r 6k -e s -b 16 -c 1 - -t wav - 2>/dev/null");
    }
    if ((strcmp(RTLmode[0], "ysf") == 0) || (strcmp(RTLmode[0], "dstar") == 0))
    {
      strcat(rtlcall, " | tee >(aplay2 -q -f S16_LE -D plughw:Loopback,0,2 -r 48) >(aplay -q -f S16_LE -D ");
    }
    else
    {
      strcat(rtlcall, " | tee >(aplay2 -q -f S16_LE -D plughw:Loopback,0,2) >(aplay -q -f S16_LE -D ");
    }
    //strcat(rtlcall, "bluealsa:DEV=");
    strcat(rtlcall, "plughw:");
    strcat(rtlcall, card);
    strcat(rtlcall, ",0");
    if ((strcmp(RTLmode[0], "ysf") == 0) || (strcmp(RTLmode[0], "dstar") == 0))
    {
      strcat(rtlcall, " -r 48) >/dev/null) &'");
    }
    else
    {
      strcat(rtlcall, ") >/dev/null) &'");
    }
...
// Paint each defined button and the title on the current Menu
{
  ...
  if (((CurrentMenu >= 42) && (CurrentMenu <= 47)) || ((CurrentMenu >= 50) && (CurrentMenu <= 54)))  // 15-button menus
  {
...
void WiFiConfig(int NoButton)
{
  ...
  GetConfigParam(PATH_HOTSPOTCONFIG,"hotspot",Hotspot);

  switch (NoButton)
  {
    case 5:                               //
      if (strcmp(Hotspot, "on") == 0)
      {
        system("/home/pi/rpidatv/scripts/hotspot_stop.sh");
      }
...
void ProcessLeandvb2(int NoButton)
{
  #define PATH_SCRIPT_LEANVLC "/home/pi/rpidatv/scripts/leandvb_vlc.sh 3>&1"
  #define PATH_SCRIPT_LEANOMX "/home/pi/rpidatv/scripts/leandvb_omx.sh 3>&1"
  #define PATH_SCRIPT_LEANUDP "/home/pi/rpidatv/scripts/leandvb_udp.sh 3>&1"
  ...
  case 0:
    ...
    while (((FinishedButton == 1) || (FinishedButton == 2))) // 1 is captions on, 2 is off
    {
      ...
      if ((read_param = getline(&line, &len, fp)) != -1)
      {
        char strTag[20];
        ...
        if (FinishedButton == 1)  // Parameters requested to be displayed
        {
          ...
          Text2(wscreen * 1 / 40, hscreen - 1 * linepitch, sLock, font_ptr);
          rectangle(wscreen * 1 / 40, hscreen - 5 * linepitch - txtdesc, wscreen * 19 / 40, txttot, 0, 0, 0);
          Text2(wscreen * 1 / 40, hscreen - 5 * linepitch, sSignalStrength, font_ptr);
  ...
  case 1:
    fp=popen(PATH_SCRIPT_LEANOMX, "r");
    if(fp==NULL) printf("Process error\n");
    ...
    printf("Stopping receive process\n");
    pclose(fp);
    system("sudo killall leandvb_omx.sh >/dev/null 2>/dev/null");
    touch_response = 0;
    printf("OMX has shut down\n");
    break;

  case 3:  // DVB-S/S2 UDP Output
    snprintf(udp_string, 63, "UDP Output to %s:%s", LMRXudpip, LMRXudpport);
    fp=popen(PATH_SCRIPT_LEANUDP, "r");
    if(fp==NULL) printf("Process error\n");
    ...
    printf("Stopping receive process\n");
    pclose(fp);
    system("sudo killall leandvb_udp.sh >/dev/null 2>/dev/null");
    touch_response = 0;
    break;
...
void ReceiveStop()
{
  ...
  if (strcmp(RXKEY, "LIMEMINI") == 0)
  {
     ...
     sleep(1);
     system("sudo killall -9 limesdr_dump >/dev/null 2>/dev/null");
     system("/home/pi/rpidatv/bin/limesdr_stopchannel");
  }
...
void SARSAT_READER()
{
  ...
  strcat(line1, date);
  Text2(wscreen / 40, hscreen - 1.75 * linepitch, line1, font_ptr);
  strcpy(line2, "ID: ");
  strcat(line2, ID);
  Text2(wscreen / 40, hscreen - 3.25 * linepitch, line2, font_ptr);
  strcpy(line3, "Latitude: ");
  strcat(line3, lat);
  strcat(line3, "   Longitude: ");
  strcat(line3, lon);
  Text2(wscreen / 40, hscreen - 4.75 * linepitch, line3, font_ptr);
  ...
  Text2(wscreen / 40, hscreen - 15.5 * linepitch, end, font_ptr);
...
void SARSAT_DECODER()
{
  ...
  while (((read = getline(&line, &len, fp)) != -1) && (FinishedButton == 0))
  {
    ...
    if(((strcmp(strTag, "Scan")==0) || (strcmp(strTag, "Lancement")==0) || (strcmp(strTag, "****Attente")==0) || (strcmp(strTag, "CRC_1")==0) || (strcmp(strTag, "CRC_2")==0) || (strcmp(strTag, "Contenu")==0)) && (Lock_GUI!=2))
    {
       ...
       {
         setForeColour(0, 0, 0);
         Text2(wscreen / 40, hscreen - 1 * linepitch, line1, font_ptr);
         strcpy(line1, line);
         setForeColour(255, 255, 255);
         Text2(wscreen / 40, hscreen - 1 * linepitch, line1, font_ptr);
       }
       ...
       if (strcmp(strTag, "Contenu")==0)
         {
           ...
           setForeColour(255, 255, 255);
           Text2(wscreen / 40, hscreen - 1 * linepitch, line1, font_ptr);
           Text2(wscreen / 40, hscreen - 1.75 * linepitch, line2, font_ptr);
           nbline=3;
           strcpy(line3, "");
           strcpy(line3, line);
           Text2(wscreen / 40, hscreen - 2.5 * linepitch, line3, font_ptr);
           strcpy(line4, "");
           strcpy(line5, "");
           strcpy(line6, "");
           strcpy(line7, "");
    ...
    }else if (Lock_GUI!=2)
    {
      if (nbline==1){
         ...
         nbline++;
         strcpy(line4, line);
         Text2(wscreen / 40, hscreen - 3.25 * linepitch, line4, font_ptr);
      }else if (nbline==4){
         nbline++;
         strcpy(line5, line);
         Text2(wscreen / 40, hscreen - 4 * linepitch, line5, font_ptr);
      }else if (nbline==5){
         nbline++;
         strcpy(line6, line);
         Text2(wscreen / 40, hscreen - 4.75 * linepitch, line6, font_ptr);
      }else if (nbline==6){
         nbline++;
         strcpy(line7, line);
         Text2(wscreen / 40, hscreen - 5.5 * linepitch, line7, font_ptr);
      }else if (nbline==7){
         nbline++;
         strcpy(line8, line);
         Text2(wscreen / 40, hscreen - 6.25 * linepitch, line8, font_ptr);
      }else if (nbline==8){
         nbline++;
         strcpy(line9, line);
         Text2(wscreen / 40, hscreen - 7 * linepitch, line9, font_ptr);
      }else if (nbline==9){
         nbline++;
         strcpy(line10, line);
         Text2(wscreen / 40, hscreen - 7.75 * linepitch, line10, font_ptr);
      }else if (nbline==10){
         nbline++;
         strcpy(line11, line);
         Text2(wscreen / 40, hscreen - 8.5 * linepitch, line11, font_ptr);
      }else if (nbline==11){
         nbline++;
         strcpy(line12, line);
         Text2(wscreen / 40, hscreen - 9.25 * linepitch, line12, font_ptr);
      }else if (nbline==12){
         nbline++;
         strcpy(line13, line);
         Text2(wscreen / 40, hscreen - 10 * linepitch, line13, font_ptr);
      }else if (nbline==13){
         nbline++;
         strcpy(line14, line);
         Text2(wscreen / 40, hscreen - 10.75 * linepitch, line14, font_ptr);
      }else if (nbline==14){
         nbline++;
         strcpy(line15, line);
         Text2(wscreen / 40, hscreen - 11.5 * linepitch, line15, font_ptr);
      }else if (nbline==15){
         nbline++;
         strcpy(line16, line);
         Text2(wscreen / 40, hscreen - 12.25 * linepitch, line16, font_ptr);
      }else if (nbline==16){
         nbline++;
         strcpy(line17, line);
         Text2(wscreen / 40, hscreen - 13 * linepitch, line17, font_ptr);
      }else if (nbline==17){
         nbline++;
         strcpy(line18, line);
         Text2(wscreen / 40, hscreen - 13.75 * linepitch, line18, font_ptr);
      }else if (nbline==18){
         nbline++;
         strcpy(line19, line);
         Text2(wscreen / 40, hscreen - 14.5 * linepitch, line19, font_ptr);
      }
    }
...
void do_Langstone()
{
  ...
  if ((DetectUSBAudio() == 0) || (strcmp(langstone_version, "v2rtlsdr") == 0) || (strcmp(langstone_version, "v2rtlhf") == 0))
  {
    ...
    if (strcmp(langstone_version, "v2rtlsdr") == 0)
    {
      if (CheckRTL() == 0)
      {
        cleanexit(155);  // Start Langstone v2 for RtlSdr
      }
      else
      {
    ...
    if (strcmp(ScreenState, "NormalMenu") == 0)
    {
      ...
      if (CurrentMenu == 5)  // Menu 5 LeanDVB
      {
        ...
        case 4:
          ...
          else if(((strcmp(RXKEY, "LIMEMINI") == 0) && (CheckLimeMiniConnect() == 0)) || ((strcmp(RXKEY, "LIMEUSB") == 0) && (CheckLimeUSBConnect() == 0)))
          {
            snprintf(ValueToSave, 63, "%d", LMRXfreq[0]);
...
*/          break;
      ...
      if (CurrentMenu == 7)  // Menu 7
      {
        ...
        case 4:                                                 // SDR Play BandViewer
          ...
          UpdateWindow();
          break;
        case 5:                                                 // Frequency Sweeper                                                 // Noise Meter
          if((CheckLimeMiniConnect() == 0) || (CheckLimeUSBConnect() == 0))
          {
            DisplayLogo();
            cleanexit(139);
          ...
          UpdateWindow();
          break;
        case 6:                                                 // Lime Noise Figure Meter                                                 // Frequency Sweeper
          if((CheckLimeMiniConnect() == 0) || (CheckLimeUSBConnect() == 0))
          {
            DisplayLogo();
            cleanexit(138);
...
//          break;
      ...
      if (CurrentMenu == 50)  // Menu 50 LeanDVB Modulation Selection
      {
        ...
        case 4:                               // Cancel
          ...
          usleep(500000);
          SelectInGroupOnMenu(CurrentMenu, 4, 4, 4, 0); // Reset cancel (even if not selected)
          printf("Returning to MENU 5 from Menu 50\n");
          CurrentMenu=5;
          setBackColour(0, 0, 0);
          Start_Highlights_Menu5();
          UpdateWindow();
        ...
        continue;   // Completed Menu 50 action, go and wait for touch
			}

      if (CurrentMenu == 51)  // Menu 51 LeanDVB SDR Selection
      {
        ...
        case 4:                               // Cancel
          ...
          usleep(500000);
          SelectInGroupOnMenu(CurrentMenu, 4, 4, 4, 0); // Reset cancel (even if not selected)
          printf("Returning to MENU 5 from Menu 51\n");
          CurrentMenu=5;
          setBackColour(0, 0, 0);
          Start_Highlights_Menu5();
          UpdateWindow();
      ...
      if (CurrentMenu == 52)  // Menu 16 Frequency
      {
        ...
        SelectInGroupOnMenu(CurrentMenu, 4, 4, 4, 0); // Reset cancel (even if not selected)
        printf("Returning to MENU 5 from Menu 52\n");
        CurrentMenu=5;
        setBackColour(0, 0, 0);
        Start_Highlights_Menu5();
        UpdateWindow();
      ...
      if (CurrentMenu == 53)  // Menu 53 LeanDVB Config
      {
      ...
      if (CurrentMenu == 55)  // Menu 55 Network Config
      {
        ...
        case 14:                              // Reset Wifi        was 5
          SetButtonStatus(ButtonNumber(55, i), 1);  // Set button green while waiting
          UpdateWindow();
          WiFiConfig(i - 5);
          SetButtonStatus(ButtonNumber(55, i), 0);
          CurrentMenu = 55;
          setBackColour(0, 0, 0);
          clearScreen();
          Start_Highlights_Menu55();
        ...
        case 19:                                            // Set RTL-SDR ppm offset
          ...
          setBackColour(0, 0, 0);
          clearScreen();
          Start_Highlights_Menu55();          // Refresh button labels
          UpdateWindow();
          break;
          break;
        case 21:                       // RX
...
void Define_Menu5()
{
  ...
  //AddButtonStatus(button, "Constel'n^OFF", &Grey);

  //button = CreateButton(5, 19);
  //AddButtonStatus(button, "Params^ON", &Blue);
  //AddButtonStatus(button, "Params^ON", &Green);
  //AddButtonStatus(button, "Params^OFF", &Grey);

  //RECEIVE and Exit - Top of Menu 5

  button = CreateButton(5, 20);
  //AddButtonStatus(button, "RX => TX^", &Blue);
  //AddButtonStatus(button, "RX => TX^", &Red);
  AddButtonStatus(button, "RX => TX^Indisponible", &Grey);
...
void Start_Highlights_Menu36()
{
  ...
  /// Bouton SSID
  system("sudo /home/pi/rpidatv/scripts/wifi_check.sh");
  strcpy(Param,"ssid");
  char Getssid[255];
  strcpy(Getssid,"");
  ...
  if (strcmp(Value, "Hotspot") != 0)
  {
    ...
    AmendButtonStatus(ButtonNumber(36, 0), 0, Getssid, &Red);
    AmendButtonStatus(ButtonNumber(36, 0), 1, Getssid, &Green);
    AmendButtonStatus(ButtonNumber(36, 0), 2, Getssid, &Orange);

    SetButtonStatus(ButtonNumber(CurrentMenu, 9), 0);

    if (strcmp(Value, "Deconnecte") == 0)
    {
      SetButtonStatus(ButtonNumber(CurrentMenu, 0), 0);
    }
    else
...
void Define_Menu50()
{
  ...
  strcpy(MenuTitle[50], "LeanDVB Modulation Selection Menu (50)");
...
void Define_Menu51()
{
  ...
  strcpy(MenuTitle[51], "LeanDVB SDR Selection Menu (51)");
...
void Define_Menu52()
{
  ...
  strcpy(MenuTitle[52], "Frequency Selection Menu (52)");
...
void Start_Highlights_Menu52()
{
  ...
  strcpy(MenuTitle[52], " Receive Frequency Selection Menu (52)");
  ...
  for(index = 0; index < 9 ; index = index + 1)
  {
    ...
    AmendButtonStatus(ButtonNumber(52, NoButton), 0, FreqBtext, &Blue);
    AmendButtonStatus(ButtonNumber(52, NoButton), 1, FreqBtext, &Green);

    //Highlight the Current Button
    if (strcmp(RXfreq[0], TabFreq[index]) == 0)
    {
      SelectInGroupOnMenu(52, 5, 9, NoButton, 1);
      SelectInGroupOnMenu(52, 0, 3, NoButton, 1);
    }
  }
}
...
void Define_Menu55()
{
  ...
  strcpy(MenuTitle[55], "Network and WiFi Config Menu (55)");
  ...
  button = CreateButton(55, 10);
  AddButtonStatus(button, "List WiFi^Networks", &Blue);
  AddButtonStatus(button, "List WiFi^Networks", &Green);
  AddButtonStatus(button, "List WiFi^Networks", &Grey);

  button = CreateButton(55, 11);
  AddButtonStatus(button, "Set-up WiFi^Network", &Blue);
  AddButtonStatus(button, "Set-up WiFi^Network", &Green);
  AddButtonStatus(button, "Set-up WiFi^Network", &Grey);

  button = CreateButton(55, 12);
  AddButtonStatus(button, "Check WiFi^Status", &Blue);
  AddButtonStatus(button, "Check WiFi^Status", &Green);
  AddButtonStatus(button, "Check WiFi^Status", &Grey);

  button = CreateButton(55, 13);
  AddButtonStatus(button, "Enable^WiFi", &Blue);
  AddButtonStatus(button, "Enable^WiFi", &Green);
  AddButtonStatus(button, "Enable^WiFi", &Grey);

  button = CreateButton(55, 14);
  AddButtonStatus(button, "Reset^WiFi", &Blue);
  AddButtonStatus(button, "Reset^WiFi", &Green);
  AddButtonStatus(button, "Reset^WiFi", &Grey);

  // 4th line up Menu 55
//  button = CreateButton(55, 19);
//  AddButtonStatus(button, "RTL ppm", &Blue);
...
void Start_Highlights_Menu55()
{
  ...
  if (CheckWifiEnabled() == 1)  // Wifi not enabled
  {
    SetButtonStatus(ButtonNumber(55, 10), 2);  // Grey out the "List WiFi Networks" Button
    SetButtonStatus(ButtonNumber(55, 11), 2);  // Grey out the "Set up WiFi Network" Button
  }
  else
  {
    if (GetButtonStatus(ButtonNumber(55, 10)) == 2) // "List WiFi Networks" is grey
    {
      SetButtonStatus(ButtonNumber(55, 10), 0);  // Set to Blue
    }
    if (GetButtonStatus(ButtonNumber(55, 11)) == 2) // "Set up WiFi Network" is grey
    {
      SetButtonStatus(ButtonNumber(55, 11), 0);  // Set to Blue
    }
  }

  //snprintf(Buttext, 26, "RTL ppm^%d", RTLppm);
  //AmendButtonStatus(ButtonNumber(55, 19), 0, Buttext, &Blue);
