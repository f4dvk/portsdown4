...
#define DEFAULT_BANDWIDTH  		1200000
...
void usage(void)
{
	fprintf(stderr,
		...
		"\t[-s samplerate (default: 2400000 Hz)]\n"
    "\t[-B RF bandwidth (default: 1200000 Hz)]\n"
...
int main(int argc, char **argv)
{
  ...
  uint32_t bandwidth = DEFAULT_BANDWIDTH;
	...
	while ((opt = getopt(argc, argv, "d:f:g:c:a:s:B:b:n:p:D:SI:F:t:")) != -1) {
		switch (opt) {
		case 'd':
			dev_query = optarg;
			break;
